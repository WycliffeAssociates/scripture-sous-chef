...
| ---------------- | ------------------------------ | -------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SSC-PUNCT-002`  | space-before-punct             | PUNCT    | scored           | Punctuation spacing that departs from the corpus's per-mark norm. Originally shipped v0.0.4 as the deterministic `punct.space-before-punct` (binary Warning, space-before only); **superseded by `punct.spacing-anomaly`** — corpus-relative and **bidirectional** (flags the minority spacing form of each mark), scored by conservative convention dominance at `Severity::Info`, so French/`pa_ulb` `? !` spacing self-suppresses (ADR 0029, amending 0014). |
| `SSC-PUNCT-003`  | repeated-punct                 | PUNCT    | scored           | Two or more consecutive punctuation marks (`,,`, `..`, `?!?`). Originally shipped v0.0.4 as the deterministic `punct.repeated-punct` (binary Warning, allow-list); **superseded by `punct.adjacency-anomaly`** — same conservative candidate extraction, but corpus-relative scoring at `Severity::Info` so established non-Latin conventions (Ethiopic `፤፤`, Arabic `۔۔`) self-suppress (ADR 0024). |
| `SSC-PUNCT-004`  | bracket-pair-balance           | PUNCT    | binary           | Per-verse imbalance of `()`, `[]`, `{}`, `«»`, `“”`, etc. Counts and depth. **Shipped v0.0.4** as `punct.bracket-balance` (quotes excluded, Info; ADR 0014); redesigned by ADR 0037 — UCD BidiBrackets inventory (non-ASCII pairs included), whole-book LIFO pairing, corpus-relative dominance scores. |
...
| `SSC-PUNCT-007`  | placeholder-text-leftover      | PUNCT    | binary           | Brackets like `[TODO]`, `[?]`, `<...>` left in text from drafting. Shipped v0.0.4 as `punct.placeholder-leftover` (ADR 0014); **retired** (ADR 0031) — the `[TODO]`/`[?]`/`<...>` literals were other-tooling cruft with no translator meaning, and its `?`/`*` runs are subsumed by `punct.adjacency-anomaly`. |
...
| `SSC-CASE-002`   | sentence-initial-case          | CASE     | binary           | Sentence-initial token begins lowercase (cased scripts only). **Shipped v0.0.4** as `case.sentence-initial-lowercase`, **default-disabled**; corpus-relative Wilson dominance verdict per terminal glyph as of ADR 0035 (Info, still opt-in per project). |
...
| `SSC-LEX-002`    | repeated-character-in-token    | LEX      | scored           | Three or more consecutive identical grapheme clusters (`heeello`) scored by corpus cluster recurrence and run-containing word recurrence. **Shipped v0.0.4** as deterministic `lex.repeated-character-run`; upgraded to aggregate-only corpus-relative Info scoring by ADR 0028. |
...
| `SSC-LEX-004`    | digit-only-or-punct-only-token | LEX      | scored           | A "word" containing only digits or only punctuation, surfacing as a token where text was expected. **Shipped v0.0.4** (punct-only half) as `lex.punct-only-token` — multi-mark/symbol wreckage only; single detached marks are spacing conventions, digit-only deferred (ADR 0014). Upgraded to aggregate-only corpus-relative Warning scoring by ADR 0030 (conventions like the ur-deva danda-substitute `\|` self-suppress; merge-conflict runs deferred to STRUCT; `?`-run mojibake owned by `hyg.replacement-run` since ADR 0034). |
