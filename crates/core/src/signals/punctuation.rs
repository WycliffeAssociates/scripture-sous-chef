//! Punctuation and spacing conventions. All rules in this module
//! operate at the **discourse level** (see `crate::discourse`) —
//! verse-level whitespace/punctuation can't be evaluated in isolation
//! because sentences cross verse boundaries. Findings are mapped back
//! to a Sid (or Sid range) at emit time.
//!
//! Statistical-first, config-as-override: each rule observes the
//! corpus's own convention and flags deviations. Config can pin a
...
/// Whitespace-spacing convention. Flags deviations from the corpus's
/// dominant inter-word spacing pattern (e.g. double-space in a
/// single-space corpus). Not yet implemented.
pub const SPACING_CONVENTION: RuleId = RuleId("punct.spacing-convention");

/// Terminal-punctuation convention. Flags deviations from the corpus's
/// dominant ellipsis/dash/emphasis form (e.g. `..` when `…` is
...
/// discourse stream without a closing partner. Finding is anchored at
/// the Sid where the imbalance becomes visible.
